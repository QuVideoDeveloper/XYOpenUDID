...
static int const kOpenUDIDRedundancySlots = 1;
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            if (oudid==nil) {
                ...
                [frequencyDict setValue:[NSNumber numberWithInt:++count] forKey:oudid];
            }
    ...
    if (openUDID==nil) {        
        ...
        if (localDict==nil) { 
            ...
            [localDict setValue:openUDID forKey:kOpenUDIDKey];
            [localDict setValue:appUID forKey:kOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:kOpenUDIDTSKey];
            if (optedOut) [localDict setValue:optedOutDate forKey:kOpenUDIDTSKey];
